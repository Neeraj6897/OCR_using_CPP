...
vector<float> NN_Layer::forward(const vector<float>& input) {
    ...
    #ifdef USE_CUDA
        if (use_cuda_) {
            cudaForward(input.data(), output.data());
            return output;
        }
    #endif

    //CPU fallback if no gpu is given
    for (int i = 0; i < output_size_; ++i) {
        float sum = biases_[i];
        for (int j = 0; j < input_size_; ++j)
            sum += input[j] * weights_[i*input_size_ + j];
        output[i] = sum;
    }
    return output;
}
...
vector<float> NN_Layer::backward(const vector<float>& gradient_output) {
    ...
    #ifdef USE_CUDA
        if (use_cuda_) {
            cudaBackward(gradient_output.data(), gradient_input.data());
...
void NN_Layer::update(float learning_rate) {
    {
        ...
        #ifdef USE_CUDA
            if (use_cuda_ && update_on_gpu_) {
                cudaUpdate(learning_rate);
...
void NN_Layer::saveWeights(const string& filename) const {
    #ifdef USE_CUDA
    const_cast<NN_Layer*>(this)->syncDeviceToHost_();
    #endif
